 ...
 */
function createWebhook(client, request, options) {
    return new async_js_1.APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => operations.CreateWebhookRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result] = await M.match(M.json(201, shared.Webhook$inboundSchema.optional()), M.nil([400, 401, 429], shared.Webhook$inboundSchema.optional()))(response, req);
    if (!result.ok) {

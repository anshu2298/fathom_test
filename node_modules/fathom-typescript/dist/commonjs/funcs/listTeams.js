 ...
 * List teams
 */
function listTeams(client, request, options) {
    return new async_js_1.APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => operations.ListTeamsRequest$outboundSchema.optional().parse(value), "Input validation failed");
    if (!parsed.ok) {
        return [(0, operations_js_1.haltIterator)(parsed), { status: "invalid" }];
    }
    const payload = parsed.value;
    const body = null;
    const path = (0, url_js_1.pathToFunc)("/teams")();
    ...
    const [result, raw] = await M.match(M.json(200, operations.ListTeamsResponse$inboundSchema.optional(), {
        key: "Result",
    }), M.nil([400, 401, 429], operations.ListTeamsResponse$inboundSchema.optional()))(response, req, { extraFields: responseFields });
    if (!result.ok) {
    ...
    const nextFunc = (responseData) => {
        const nextCursor = (0, dlv_js_1.dlv)(responseData, "next_cursor");
        ...
        const nextVal = () => listTeams(client, {
            ...request,
            cursor: nextCursor,
        }, options);
